...
def extract_pdf(path: str) -> str:
    ...
    full = "\n\n".join(pages)
    print(f"\n[PDF] Done — {len(full.split())} words extracted")
    return full
...
class EmbeddingModel:
    ...
    def fit_encode(self, texts):
        ...
        # SVD n_components must be < n_features; cap it safely
        n_features = mat.shape[1]
        actual_n = min(self.n, n_features - 1)
        self.svd = TruncatedSVD(n_components=actual_n, random_state=42)
        self.svd.fit(mat)
...
print(f"[Pipeline] Ready — {len(chunks)} chunks, dim={dim}\n")
...
def ask():
    ...
    if not LLM_API_KEY:
        return jsonify({"error": "LLM_API_KEY not set on server"}), 500
...
@app.route("/health")
def health():
    return jsonify({"status": "ok", "chunks": len(chunks), "dim": dim})
